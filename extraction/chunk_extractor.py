...
class ChunkExtractor:
    ...
    def compute_heightmap(self, block_types: np.ndarray) -> np.ndarray:
        ...
        # For each column, find the highest non-air block
        for x in range(16):
            for z in range(16):
                column = block_types[x, z, :]
                
                # Find surface blocks (non-air or specific surface blocks)
                surface_positions = np.where(
                    np.logical_or(
                        ~np.isin(column, list(self.air_blocks)),
                        np.isin(column, list(self.surface_blocks))
                    )
                )[0]
                
                if len(surface_positions) > 0:
                    heightmap[x, z] = surface_positions[-1]  # Highest surface block
                else:
                    heightmap[x, z] = 0  # Ground level if no surface
    ...
    def extract_regions_parallel(self, region_files: List[Path], num_workers: Optional[int] = None) -> List[str]:
        """
    ...
    def validate_extraction_results(self, output_dir: Path) -> Dict[str, Any]:
        ...
        for npz_file in npz_files:
            try:
                ...
                required_fields = ["block_types", "air_mask", "biomes", "heightmap", 
                                 "chunk_x", "chunk_z", "region_file"]
