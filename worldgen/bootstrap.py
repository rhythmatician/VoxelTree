...
class WorldGenBootstrap:
    ...
    def __init__(self, seed: str = "VoxelTree", java_heap: str = "4G", 
                 temp_world_dir: Union[str, Path] = None):
        """
    ...
    def generate_region_batch(self, x_range: Tuple[int, int], 
                            z_range: Tuple[int, int]) -> Path:
        """
        ...
        if self._get_directory_size_gb(self.temp_world_dir) > 5.0:
    ...
    def _run_worldgen_java(self, world_dir: Path, x_range: Tuple[int, int], 
        ...
        cmd = [
            ...
            "--region-x", f"{x_range[0]//32},{x_range[1]//32}",
            "--region-z", f"{z_range[0]//32},{z_range[1]//32}",
        ]
