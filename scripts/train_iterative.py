...
from train.dataset import TrainingDataCollator, VoxelTreeDataset  # noqa: E402
...
def get_current_seed_and_region(
    ...
    """
    # Calculate grid size from regions per seed
...
def generate_single_region_batch(
    ...
    try:
        ...
        bootstrap = WorldGenBootstrap(
            ...
            defer_cleanup=True,  # Don't delete temp world until training is done
        )
...
def run_iterative_training(
    ...
    try:
        # Create a bootstrap instance just for cleanup
        cleanup_bootstrap = WorldGenBootstrap(seed="cleanup")
        cleanup_bootstrap.cleanup_temp_worlds(keep_latest=0)  # Clean up all temp worlds
        logger.info("Cleaned up temporary world directories")
    except Exception as e:
        logger.warning(f"Failed to clean up temporary worlds: {e}")
...
def main():
    ...
    parser.add_argument(
        "--regions-per-seed",
        type=int,
        default=1369,  # 37x37 grid covering stronghold rings 1-3
        help="Number of regions per seed (1369=37x37 grid covers first 3 stronghold rings)",
    )
