...
class VanillaChunkGenerator:
    ...
    def launch_fabric_server(
        ...
        if not fabric_jar.exists():
            raise FileNotFoundError(f"Fabric server JAR not found at {fabric_jar}")
    ...
    def generate_vanilla_region(
        ...
        """
        # GREEN phase: minimal implementation to pass test
        # Test expects this to return a path but not actually create files yet
        if seed is None:
            seed = self.seed_numeric

        world_dir = self.temp_world_dir / f"world_{seed}_{x_range[0]}_{z_range[0]}"
        return world_dir
