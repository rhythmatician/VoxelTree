...
class TestRefinementModels:
    ...
    def test_invalid_output_size_raises(self, config):
        # ProgressiveLODModel has fixed output_size based on factory
        # Test that factories create models with correct output_size
...
class TestModelFamily:
    def test_four_factories_produce_distinct_models(self, config):
        ...
        assert type(m1) is TinyAttentionUpscaler
        assert type(m2) is BaselineConv3D4Cube
        assert type(m3) is TinyUNet8Cube

        # All refinement models inherit from ProgressiveLODModel
        assert isinstance(m1, ProgressiveLODModel)
        assert isinstance(m2, ProgressiveLODModel)
        assert isinstance(m3, ProgressiveLODModel)
